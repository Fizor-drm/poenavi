...
class LogWatcher(QObject):
    ...
    def __init__(self, log_path: str = "", poll_interval_ms: int = 500, parent=None):
        ...
        self._active = False
        self._twilight_capture_started_at = None
        self._twilight_capture_path = None

    def _twilight_trace_enabled(self) -> bool:
        return os.environ.get("POENAVI_TWILIGHT_TRACE") == "1"

    def _twilight_capture_seconds(self) -> float:
        try:
            return max(
                0.0,
                float(os.environ.get("POENAVI_TWILIGHT_CAPTURE_SECONDS", "120")),
            )
        except ValueError:
            return 120.0

    def _start_twilight_capture(self):
        if not self._twilight_trace_enabled():
            return
        if self._twilight_capture_started_at is not None:
            return

        user_data_dir = Path(
            os.environ.get("POENAVI_USER_DATA_DIR")
            or Path.cwd() / ".dev-user-data"
        )
        trace_dir = user_data_dir / "diagnostics"
        trace_dir.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.now().astimezone().strftime("%Y%m%d-%H%M%S")
        self._twilight_capture_path = (
            trace_dir / f"twilight-client-log-{timestamp}.txt"
        )
        self._twilight_capture_started_at = time.perf_counter()
        header = (
            "# 黄昏の岸辺以降のClient.txt診断\n"
            f"# 開始時刻: {datetime.now().astimezone().isoformat(timespec='milliseconds')}\n"
            f"# 保存時間: {self._twilight_capture_seconds():g}秒\n"
            "# 形式: [検知からの経過時間] [PoENavi受信時刻] Client.txt原文\n"
        )
        self._twilight_capture_path.write_text(header, encoding="utf-8")
        print(
            f">>> 以降のClient.txtを保存します: {self._twilight_capture_path}",
            flush=True,
        )

    def _capture_twilight_line(self, line: str):
        if self._twilight_capture_started_at is None:
            return

        elapsed = time.perf_counter() - self._twilight_capture_started_at
        capture_seconds = self._twilight_capture_seconds()
        if elapsed > capture_seconds:
            print(
                f">>> [黄昏の岸辺・検知テスト] "
                f"{capture_seconds:g}秒間のログ保存を終了しました",
                flush=True,
            )
            self._twilight_capture_started_at = None
            return
        ...
        record = f"[+{elapsed:09.3f}s] [{received_at}] {line.rstrip()}\n"
        try:
            with self._twilight_capture_path.open("a", encoding="utf-8") as trace:
                trace.write(record)
        except OSError as exc:
            print(f">>> 診断ログの保存に失敗しました: {exc}", flush=True)
            self._twilight_capture_started_at = None
            return
        print(f">>> [黄昏+{elapsed:07.3f}s] {line.rstrip()}", flush=True)

    def _trace_twilight_strand(self, zone_name: str, line: str, source: str):
        """開発用: 黄昏の岸辺を検知した瞬間をコンソールへ明示する。"""
        if not self._twilight_trace_enabled():
            return
    ...
    def _poll(self):
        ...
        try:
            ...
            for line in lines:
                ...
                self._capture_twilight_line(line)
